...
- **Right-click a task line** in a project note or the inbox (long-press on mobile) for a context menu: **Edit task**, **Complete task** (writes ✅ and the 🔁 next occurrence), **Drop task…** (prompts for a 💬 reason when that setting is on), and **Mark/Remove someday**.
...
1. `#important` marker with an Important perspective
2. Folder-note overview block (aggregated project stats when Folder Notes is installed)
